...
MatrixXf QuadEstimatorEKF::GetRbgPrime(float roll, float pitch, float yaw)
{
  ...
  float cosTheta = std::cosf(pitch);
  float sinTheta = std::sinf(pitch);
  float cosPhi   = std::cosf(roll);
  float sinPhi   = std::sinf(roll);
  float cosPsi   = std::cosf(yaw);
  float sinPsi   = std::sinf(yaw);

  RbgPrime(0, 0) = -cosTheta * sinPsi;
  RbgPrime(0, 1) = sinPhi * sinTheta*sinPsi - cosPsi * cosPhi;
